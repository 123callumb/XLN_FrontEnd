    ...
    submitData(){
        (async() => {
            ...
            try{
                ...
                if(response.data){
                    const missedFields = "Not all field are filled in " + data.map((e) => {'\n' + e});
                    this.setState({errorMsg: missedFields, errorUI: true});
                }else{
                    console.log("Success, what it actually worked LOL?");
                    this.props.disableHandler();
                }
            }catch(e){
                console.log("Insert business data Error " + e);
                this.setState({errorMsg: "Server connection error", errorUI: true});
            }
